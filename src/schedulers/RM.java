...
public class RM{
  ...
  public int[] Shedule(){
    ...
    while(ctime<=fin){
      UpdateReady();
      taskexecute=TaskShortestPeriod();
      if(taskexecute == -1){
  ...
  public void ShowGeneral(){
    ...
    for(int i=0;i<n;i++){
      ...
      System.out.println("s "+aux.start+" : c "+aux.timecompt+" : p "+aux.period+" : d "+aux.deadline+" ");
    }
  }
...
public void UpdateReady(){
  ...
  public int TaskShortestPeriod(){
    ...
    for(int i=0;i<tam;i++)
        ...
        }else if(readytasks.get(i).period<aux){
          aux=readytasks.get(i).period;
          shortest=i;
        }
    return shortest;
...
/*
  public static void main(String[] args){
    ...
    for(int i=0;i<n;i++){
      ...
      d=lee.nextInt();
      rm.AddTask(new PeriodTask(s,d,t,i));
    }
